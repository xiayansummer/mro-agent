...
async def handle_message(
    ...
    # Step 2: Search SKUs (always search, even when clarification needed)
    yield f"event: thinking\ndata: 正在搜索产品...\n\n"

    results = await search_skus(db_session, parsed, limit=20)

    # Fallback with relaxed search if no results
    if not results:
        results = await relaxed_search(db_session, parsed, limit=20)

    # Attach file information from t_item_file_sample
    ...
    if results and need_clarification:
        # Broad results found + need more details → show results with guidance
